...
| **航线** | 五段航程工作流：每段由对应船员起草一份可直接使用的交付物（策划书/侦察报告/造船清单/发布包/增长信风），支持重新起草与导出 .md；**另一名船员按 rubric 互审评分**（可执行性/具体性/定位一致性），高优问题自动修订，薄弱项支持定向修订 |
| **船坞** | 生成**单文件落地页**（HTML），内嵌实时预览、下载、直接部署 |
| **信号塔** | 按渠道语气生成推广文案（小红书/朋友圈/校园公告/知乎），一键复制 |
| **藏书舱** | 本地知识库：粘贴任意文本自动分段；生成时检索最相关段落注入上下文（本地 RAG，bigram 打分，检索试验台可解释）；**产物中的结论可带 [资料N] 徽章，点击回跳来源段落，无依据推断标注「待验证」** |
| **航海志** | append-only 项目日志 + **OPC 成本仪表盘**（按航段实测调用次数/耗时/tokens/费用），一键导出 .md —— OPC 迭代速度与零成本的故事都有据可查 |
| **罗盘** | 引擎接口配置、连接测试、数据清空 |
...
python _check.py        # 走通：起航 → 立意/侦察生成 → 落地页 → 文案 → 知识库入库与检索 → 航海志 .md 导出下载（断言产物） → 罗盘
```

截图输出在 `_shots/`。最近一次全流程自检：9 张截图全部通过，控制台 0 报错，导出文件断言通过。
